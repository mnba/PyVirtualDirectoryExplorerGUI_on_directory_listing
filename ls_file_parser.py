...
class lslaRfile:
	...
	def __init__(self,file):
		...
		self.refile= re.compile("(.{10})\s+(\d+)\s+(\S)+\s+(\S+)\s+(\d+) (\S+)\s+(\d+)\s+(\d\d):(\d\d) (\S.*)")
		self.refile2= re.compile("(.{10})\s+(\d+)\s+(\S)+\s+(\S+)\s+(\d+) (\S+)\s+(\d+)\s+(\d\d\d\d) (\S.*)")
		self.read(file)
	...
	def read(self,filename):
		...
		print "lslaRfile.read(): %s: %d lines found, %d errors."%(filename,self.lines_cnt, err_cnt)
		if len(err_lst)>0:
			for l in err_lst: print "'%s'"% l
		self.f.close()
